...
def run_epoch(student, teacher, regressor,
    ...
    n = total
    return total_loss / n, total_ce / n, total_mmd / n, correct / n
...
def main():
    ...
    print(f"  {'Epoch':>5}  {'LR':>8}  "
          f"{'Tr-Loss':>8} {'Tr-CE':>7} {'Tr-MMD':>7} {'Tr-Acc':>7}  "
          f"{'Va-Loss':>8} {'Va-Acc':>7}")
    print(f"{'='*75}")

    for epoch in range(1, EPOCHS + 1):
        tr_loss, tr_ce, tr_mmd, tr_acc = run_epoch(
            student, teacher, regressor,
            train_loader, optimizer, device, epoch, is_train=True)

        va_loss, va_ce, va_mmd, va_acc = run_epoch(
            student, teacher, regressor,
        ...
        print(f"  {epoch:5d}  {lr_now:8.2e}  "
              f"{tr_loss:8.4f} {tr_ce:7.4f} {tr_mmd:7.4f} {tr_acc:7.4f}  "
    ...
    te_loss, te_ce, te_mmd, te_acc = run_epoch(
        student, teacher, regressor,
