...
from multimodal.model.Teacher1 import TeacherModel, TeacherLoss
...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return (self.eeg[idx], self.audio[idx], self.vision[idx], self.y[idx])
...
def build_loaders(vision_max_len: Optional[int] = None):
    ...
    (tr_eeg, tr_y, _), (va_eeg, va_y, _), (te_eeg, te_y, _) = \
    ...
    def make(eeg, aud, vis, y, shuffle):
        ds = MultiModalDataset(eeg, aud, vis, y, vision_max_len)
        return DataLoader(ds, batch_size=BATCH_SIZE, shuffle=shuffle,
                          num_workers=4, pin_memory=True)

    return (make(tr_eeg, tr_aud, tr_vis, tr_y, True),
            make(va_eeg, va_aud, va_vis, va_y, False),
            make(te_eeg, te_aud, te_vis, te_y, False))
...
def run_epoch(model, loader, criterion, optimizer, device, is_train, epoch):
    ...
    with ctx:
        for eeg, audio, vision, labels in pbar:
...
def main():
    ...
    best_eeg_acc   = 0.0
    patience_count = 0
    ckpt_path      = os.path.join(SAVE_DIR, 'best_teacher1.pth')
